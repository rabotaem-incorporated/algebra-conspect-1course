...
#proof[
    ...
    Аналогично для $h in H_2$.
 
    
    - "$<==$": Пусть $h_1 in H_1, h_2 in H_2$, и $h_1 h_2 != h_2 h_1$, а следовательно $h_1 h_2 h_2^(-1) h_1^(-1) eq.not e$
    $
...
#def[
    ...
    $(g_1, g_2) dot (g_1 ', g_2 ') = (g_1 g_1 ', g_2 g_2 ')$
...
#proof[
    ...
    Это получилось по свойствам внутреннего произведения
    
    - $phi$ --- инъекция тогда и только тогда, когда такие $h_1$ и $h_2$ единственны, это можно понять из предыдущего предложения

    - Остается проверить, что $phi$ --- гомоморфизм.
        $
            phi((h_1, h_2) dot (h_1 ', h_2 '))
            &= phi((h_1 h_1 ', h_2 h_2 ')) = \
            &= h_1 h_1 ' h_2 h_2' underbrace(=, #[коммутативность\ внутр. произв.]) h_1 h_2 dot h_1 ' h_2 ' =\
...
#def[
    ...
    Обозначается $G = H_1 times.three.l H_2$ если $H_1 nsubg G$ или $G = H_1 times.three.r H_2$ если $H_2 nsubg G$ и $G = H_1 times H_2$ если выполняется оба условия, что то же самое, что внутреннее прямое произведение. 
]
