...
#proof[
    Рассмотрим (здесь идет речь о множестве левых смежных классов, $St_x$ не обязан быть нормальной подгруппой!)
    $
...
#ticket[Центр конечной $p$-группы]
...
#proof[
    Пусть $G$ действует на себе сопряжением ($(g, x) maps g x g^(-1)$). Рассмотрим $h in G$. Тогда
    $
        G = union.big_(i in I) C_i, "где" C_i #[--- орбиты], \
        abs(G) space dots.v space abs(C_i) ==> abs(C_i) = p^m "для" 0 <= m <= n.
    $
    Обозначим за $k_i$ число орбит размера $p^i$. Тогда 
    $
        p^n = abs(G) = k_0 mul 1 + k_1 mul p + dots + k_n mul p^n ==> p bar k_0 \
        k_0 >= 1, "так как орбита" e "состоит только из " e", но" p >= 2
    $
