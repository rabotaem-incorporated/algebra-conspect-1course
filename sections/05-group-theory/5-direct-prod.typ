...
#def[
    Пусть $H_1, H_2$ --- подгруппы $G$, говорят, что $G$ расскладывается в _прямое произведение_ $H_1$ и $H_2$, если:
    + $G = H_1 H_2$
    + $H_1 sect H_2 = {e}$
...
#pr[
    ...
    Тогда такое пребразование $phi$ --- изоморфизм.
]
...
#def[
    ...
    Обозначается $G = H_1 times.three.l H_2$, (если $H_1 nsubg G$) или $G = H_1 times.three.r H_2$, (если $H_2 nsubg G$) и $G = H_1 times H_2$, (если выполняется оба условия?). 
    // да, тут коллизия с обозначением прямого произведения
]
