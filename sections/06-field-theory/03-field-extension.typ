...
#def[
    Размерность расширения обозначается $dim_K L = [L : K]$ и называется _степенью расширения_ $L fg K$. Если $[L : K] < oo$, то такое расширение называется _конечным_.
]
...
#denote[
    Такой многочлен обозначается $f_0 = Irr_K a$, его называют _минимальным неприодимым многочленом_ элемента $a$ над $K$.
]
...
#proof[
    + #[
        ...
        $
            ...
            Ker(phi) &= (f)
        $
        По теореме о гомоморфизме существует изоморфизм
        $
        ...
        $f$ --- неприводим, поэтому $K[x] fg (f)$ --- поле, значит $Im phi$ тоже поле. $underbrace(K, (c = phi(c), c in K)) subset Im phi, space underbrace(a, a = phi(x)) in Im phi$. 
    ]
...
#pr[
    Пусть $a$ трансцендентен над $K$. Тогда существует изоморфизм 
    $
        K(x) &--> K(a) \
        f / g &maps f(a) / g(a)\
    $
