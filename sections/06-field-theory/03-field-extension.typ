...
#denote[
    Такое многочлен обозначается $f_o = Irr_K a$.
]
...
#def[
    Расширение называют _алгебраическим_, если все элементы, добавляемые к пол. $K$ алгебраичны над $K$. Иначе расширение называется _трансцендентным_.
...
#pr[
    ...
    Где $Irr_K a$ --- минимальный многочлен $a$ над $K$, то есть неприводивый многочлен минимальной степени с коэффициентами из $K$, такой что $f(a) = 0$.
]
...
#proof[
    ...
    В $K[x] fg (f)$ есть $K$-базис, $1, overline(x), overline(x)^2, overline(x)^(d-1)$. Если бы они были линейно независимы, то 
    $
        alpha_0 + alpha_1 overline(x)^1 + ... + alpha_(d-1) overline(x)^(d-1) = 0 = overline(alpha_0 + ... + alpha_(d - 1) x^(d - 1)).
    $
    Но тогда $overline(alpha_0 + ... + alpha_(d - 1) x^(d - 1)) divides alpha_0 + ... + alpha_(d - 1) x^(d - 1)$, но так как степень многочлена справа обязана быть меньше $d$, то значит она равна 0. Значит система линейно независима.

    Рассмотрим любой элемент фактор кольца $overline(g) in K[x] fg (f)$. 
    $
        g = f dot q + r. space deg(r) <= d-1 ==> overline(g) = F in Lin(1, overline(x), overline(x)^2, ..., overline(x)^(d-1)) ==> \
        ==> overline(phi(1)), overline(phi(overline(x))), overline(overline(x)^2), ..., overline(overline(x)^(d-1))
        ==> 1, a, a^2, ..., a^(d-1) #[--- базис L]
    $
]

#example[
    $QQ(root(3, 2))$ --- простое алгебраическое расширение $!Q$.
