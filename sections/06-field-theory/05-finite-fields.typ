...
#lemma[
    Пусть $L / K$ --- конечное расширение конечного поля. Тогда $L fg K$ --- простое.
]
#proof[
...
#proof[
    ...
    В $K_2$ все элементы --- корни $x^p^n - x$, значит у него есть $p^n$ корней. Значит $x^p^n - x$ раскадывается на линейные множители в $K_2$. Получается, что $g$ раскладывается на линейные множители в $K_2$. Поэтому $exists Theta_2 in K: g(Theta_2) = 0 ==> [FF_p(Theta_2) : FF_p] = n = [K_2 : FF_p] ==> FF_p(Theta_2) = K_2$.
...
#proof[
    ...
    + $m divides n ==> (p^m - 1) divides (p^n - 1), (p^m limits(ident)_(p^m - 1) 1)$
    $ ==> (x^(p^m - 1) - 1) divides (x^(p^n - 1) - 1) ==> (x^(p^m) - x) divides (x^(p^n) - x) $ // todo тут какое-то еще сравнение было, но буквы не вижу
    ...
    $abs(A u t(FF_(p^n))) = n, space A u t(FF_(p^n)) = gen(Phi)$
    $x limits(maps)^(Phi^n) x^(p^n) = x, space Phi: x maps x^p$ // короче я не дописал.................
    $[L : K] = n ==> abs(underbrace(A u t(L fg K), {sigma in A u t L})) <= n$
]
