...
#th(name: "Принцип Дирихле для линейных отображений")[
    Пусть $Aa in Hom(V, W)$, $dim V = dim W < infinity$. Тогда $Aa$ --- инъективно равносильно тому, что $Aa$ сюрьективно.
]

#proof[
    $Aa$ --- инъективно равносильно $Ker Aa = 0 <==> dim Ker A = 0 <==> dim Im Aa = n <==> Im Aa = W$ равноситльно $Aa$ сюрьективно.
]
...
#props[
    Пусть $Aa in End V$, $dim V = n < infinity.$ Тогда следующие утверждения эквивалентны
...
#props[
    Пусть $E, F$ --- базисы пространства $V$, $dim(V) = n < infinity$, $C = M_(E arrow F)$. Тогда
