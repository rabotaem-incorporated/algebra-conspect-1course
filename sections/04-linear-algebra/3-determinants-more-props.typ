...
#proof[
  ...
  Общий случай получается, если применить элементаные преобразования $S_(i, i-1)$ над строками и столбцами. Знак суммарно поменятеся $i_0 + j_0 - 2$ раза, значит $det(A) = a_(i_0 j_0) A(i_0 j_0)$
]

#pr(name: "Разложение определителя по строке")[
