...
#proof[
   ...
   - "$1 => 3$": $alpha_1 v_1 + ... + alpha_m v_m = 0$.
     ...
     Существует $j : alpha_j != 0$. Возьмем максимальное такое $j$.
     Выкинем все слагаемые с индексом больше $j$ и поделим на $alpha_j$:
    $ 
    alpha_(1) / alpha_j v_1 + ... + v_j = 0 
    ==> v_j = -alpha_(1) / alpha_j v_(1) - ... - alpha_(j - 1) / alpha_j v_(j - 1).
    $
    
    Получилось что $v_j in Lin(v_(1), ..., v_(j - 1))$.
]
...
#proof[
  ...
  "Переход": $m - 1 ~~> m$: Знаем, что $sq(w) in Lin(sq(v, m))$, значит:
  $
  ...
  Значит $w_1, ..., w_n$ --- ЛЗС (т.к. если раскрыть скобки в формуле выше, то получатся коэфициенты для всех $w_i$).
]
